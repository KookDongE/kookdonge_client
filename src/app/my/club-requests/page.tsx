...
import { useMyApplications, useMyDeletionRequests } from '@/features/club/hooks';

function DeletionRequestsSection() {
  const { data: deletionList, isLoading, isError } = useMyDeletionRequests();
  ...
  return (
    <div className="px-4 pb-6">
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center rounded-xl border border-zinc-200 bg-zinc-50 py-10 text-zinc-400 dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-500">
          <p className="text-sm">삭제 신청한 동아리가 없습니다.</p>
        </div>
      ) : (
        <div className="space-y-3">
          {list.map((item) => (
            <Link
              key={item.requestId}
              href={`/mypage/clubs/${item.clubId}/manage`}
