...
import { ALLOWED_IMAGE_EXTENSIONS, validateImageFile } from '@/lib/image-upload-validation';
...
export const feedApi = {
  ...
  uploadFeedFiles: async (
    ...
    async function uploadOne(file: File): Promise<{ uuid: string; fileUrl: string }> {
    ...
    const results: Array<{ uuid: string; fileUrl: string }> = [];
    for (let i = 0; i < files.length; i += CONCURRENCY) {
      const chunk = files.slice(i, i + CONCURRENCY);
      const chunkResults = await Promise.all(
        chunk.map((file) =>
          uploadOne(file).catch((err) => {
