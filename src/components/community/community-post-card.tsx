...
import type { BoardType, CommunityPost } from '@/features/community/types';
...
/** 에브리타임 스타일 게시글 카드: 상단 메타(작성자·게시판·시간) → 제목 → 본문 미리보기 → 구분선 → 공감/저장/댓글 */
export function CommunityPostCard({ post, boardHref }: CommunityPostCardProps) {
  ...
  const cardContent = (
    <article className="border-b border-zinc-100 bg-white px-4 py-4 dark:border-zinc-800 dark:bg-zinc-900">
      {/* 상단: 작성자 · 게시판 · 시간 (에브리타임 메타 스타일) */}
      <div className="mb-1.5 flex items-center gap-1 text-xs text-zinc-400 dark:text-zinc-500">
      ...
      {/* 구분선 + 하단: 공감(좋아요) / 저장 / 댓글 (에브리타임 하단 스타일) */}
      <div className="mt-3 flex items-center gap-4 border-t border-zinc-100 pt-3 text-[12px] text-zinc-400 dark:border-zinc-800 dark:text-zinc-500">
