...
/** 제목 → 본문 한 줄 → 아이콘(좋아요/댓글/저장) + 시간 + 작성자 */
export function CommunityPostCard({ post, boardHref }: CommunityPostCardProps) {
  ...
  const cardContent = (
    <article className="border-b border-zinc-100 bg-white px-4 py-3 dark:border-zinc-800 dark:bg-zinc-900">
      {/* 제목 */}
      ...
      <div className="mt-2 flex items-center gap-3 text-[12px] text-zinc-400 dark:text-zinc-500">
        ...
        <span className="flex items-center gap-1.5" aria-label={`저장 ${post.saveCount}개`}>
          <svg
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 24 24"
