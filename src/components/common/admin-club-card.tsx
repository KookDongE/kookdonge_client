...
export function AdminClubCard({ club, index = 0, onToggleVisibility, onDelete }: AdminClubCardProps) {
  ...
  return (
    <div className="relative overflow-hidden">
      ...
      {/* 숨기기/삭제 버튼 (카드 외부 우측에 배치) */}
      <div
        className={`absolute right-0 top-0 flex h-full flex-col justify-center gap-2 px-2 py-2 transition-opacity ${
      ...
      >
        <button
          ...
          className="pointer-events-auto flex flex-col items-center justify-center gap-1 rounded-2xl bg-gray-200 px-2.5 py-1.5 text-gray-700 transition-colors hover:bg-gray-300 dark:bg-zinc-700 dark:text-zinc-300 dark:hover:bg-zinc-600"
        >
        ...
        <button
          ...
          className="pointer-events-auto flex flex-col items-center justify-center gap-1 rounded-2xl bg-red-100 px-2.5 py-1.5 text-red-600 transition-colors hover:bg-red-200 dark:bg-red-950/30 dark:text-red-400 dark:hover:bg-red-950/50"
        >
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} className="h-4 w-4">
