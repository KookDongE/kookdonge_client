    ...
    login.mutate(
      { googleGrantCode: 'demo' },
      {
        onSuccess: (res) => {
          ...
          const isFirstLogin = typeof window !== 'undefined' && !localStorage.getItem(WELCOME_SEEN_KEY);
          router.replace(isFirstLogin ? '/welcome' : '/home');
        },
        onError: () => alert('로그인에 실패했습니다. 다시 시도해 주세요.'),
      }
    );
  };
    ...
    <div className="flex min-h-dvh flex-col items-center justify-center bg-[var(--background)] px-6">
      ...
      >
        ...
        >
          ...
          {login.isPending ? '로그인 중...' : 'Google로 로그인'}
        </motion.button>
      </motion.div>
    </div>
  );
