...
## 참고: "파일에 대한 접근 권한이 없습니다"

커뮤니티 글 이미지는 **항상 POST /api/files** 로만 등록합니다. 동아리 계정으로 쓴 글도 마찬가지입니다.

- **POST /api/clubs/{clubId}/files** 로 등록하면 백엔드가 해당 파일을 "동아리 소속"으로 간주해, 이미지 조회 시 동아리 멤버만 허용하는 등 접근 제어를 걸 수 있습니다. 그러면 커뮤니티 글에 붙은 이미지를 다른 사용자가 볼 때 "파일에 대한 접근 권한이 없습니다"가 발생합니다.
- 따라서 커뮤니티용 이미지는 **POST /api/files** 로만 올리고, body의 `clubId`는 `file_information.club_id` 저장용으로만 쓰고, **파일 접근 권한은 "커뮤니티 공개"**로 두어야 합니다 (동아리 멤버만 보이게 하면 안 됨).
